...
fn _unchecked_zero[D: DType, size: Int]() -> SIMD[D, size]:
    ...
    return SIMD[D, size] {
        value: __mlir_op.`pop.simd.splat`[
            _type = __mlir_type[`!pop.simd<`, size.value, `, `, D.value, `>`]
        ](zero)
    }
...
struct SIMD[D: DType, size: Int](
    ...
    alias type = D
    alias element_type = D
    var value: __mlir_type[`!pop.simd<`, size.value, `, `, D.value, `>`]
    """The underlying storage for the vector."""

    alias MAX = Self(_max_or_inf[D]())
    """Gets the maximum value for the SIMD value, potentially +inf."""

    alias MIN = Self(_min_or_neg_inf[D]())
    """Gets the minimum value for the SIMD value, potentially -inf."""

    alias MAX_FINITE = Self(_max_finite[D]())
    """Returns the maximum finite value of SIMD value."""

    alias MIN_FINITE = Self(_min_finite[D]())
    """Returns the minimum (lowest) finite value of SIMD value."""

    alias _default_alignment = alignof[Scalar[D]]() if is_gpu() else 1
    ...
    fn __init__(out self):
        ...
        """
        _simd_construction_checks[D, size]()
        self = _unchecked_zero[D, size]()

    # FIXME(MOCO-1291): Can't implement this due to ambiguity.
    # @always_inline("nodebug")
    # fn __init__(out self, *, other: SIMD[D, size]):
    ...
    fn __init__(out self, value: __mlir_type.index):
        ...
        var casted = __mlir_op.`pop.cast`[
            _type = __mlir_type[`!pop.simd<1,`, D.value, `>`]
        ](t0)
    ...
    fn __init__(
        inout self,
        value: __mlir_type[`!pop.simd<`, size.value, `, `, D.value, `>`],
    ):
    ...
    @always_inline("nodebug")
    @implicit
    fn __init__(out self, value: Scalar[D], /):
    ...
    @always_inline("nodebug")
    @implicit
    fn __init__(out self, *elems: Scalar[D]):
        ...
        self = __mlir_op.`kgen.param.constant`[
            ...
            value = __mlir_attr[
                `#kgen.unknown : `,
                __mlir_type[`!pop.simd<`, size.value, `, `, D.value, `>`],
            ],
        ]()
    ...
    fn __init__(out self, value: FloatLiteral):
        ...
        _simd_construction_checks[D, size]()
        constrained[
            D.is_floating_point(), "the SIMD type must be floating point"
        ]()

        # TODO (#36686): This introduces unneeded casts here to work around
        # parameter if issues.
        @parameter
        if D is DType.float16:
            self = SIMD[D, size](
                __mlir_op.`pop.simd.splat`[
                    _type = __mlir_type[
                        `!pop.simd<`, size.value, `,`, D.value, `>`
                    ]
                ](
                    __mlir_op.`pop.cast`[
                        _type = __mlir_type[`!pop.scalar<`, D.value, `>`]
    ...
    @always_inline("nodebug")
    fn __getitem__(self, idx: Int) -> Scalar[D]:
    ...
    @always_inline("nodebug")
    fn __setitem__(inout self, idx: Int, val: Scalar[D]):
    ...
    fn __contains__(self, value: Scalar[D]) -> Bool:
    ...
    fn __add__(self, rhs: Self) -> Self:
        ...
        Returns:
            ...
            `self[i] + rhs[i]`.
        """
        constrained[D.is_numeric(), "the SIMD type must be numeric"]()

        @parameter
        if _is_sm_8x() and D.is_half_float():
    ...
    fn __sub__(self, rhs: Self) -> Self:
        ...
        @parameter
        if _is_sm_9x() and D is DType.bfloat16:
        ...
        elif _is_sm_8x() and D.is_half_float():
    ...
    fn __floordiv__(self, rhs: Self) -> Self:
        ...
        """
        constrained[D.is_numeric(), "the type must be numeric"]()
    ...
    fn __pow__(self, exp: Int) -> Self:
        ...
        """
        constrained[D.is_numeric(), "the SIMD type must be numeric"]()
        return _pow[D, size, DType.index](self, exp)
    ...
    fn __eq__(self, rhs: Self) -> Self._Mask:
        ...
        @parameter
        if has_neon() and D is DType.bfloat16:
    ...
    fn __and__(self, rhs: Self) -> Self:
        ...
        """
        constrained[
            D.is_integral() or D is DType.bool,
    ...
    fn __lshift__(self, rhs: Self) -> Self:
        ...
        Returns:
            `self << rhs`.
        """
        constrained[D.is_integral(), "must be an integral type"]()
        debug_assert(all(rhs >= 0), "unhandled negative value")
        debug_assert(
            all(rhs < bitwidthof[D]()), "unhandled value greater than size"
        )
    ...
    fn __invert__(self) -> Self:
        ...
        """
        constrained[
            D is DType.bool or D.is_integral(),
    ...
    fn __int__(self) -> Int:
        ...
        alias type_width = bitwidthof[D]()

        @parameter
        if D.is_unsigned() and int_width > type_width:
    ...
    fn __mlir_index__(self) -> __mlir_type.index:
        ...
        """
        constrained[
            D.is_integral(), "cannot index using a floating point type"
        ]()
    ...
    fn __repr__(self) -> String:
        ...
        """
        var output = String()
        output.write("SIMD[" + D.__repr__() + ", ", size, "](")
        # Write each element.
        for i in range(size):
            var element = self[i]
    ...
    fn __abs__(self) -> Self:
        ...
        @parameter
        if D.is_unsigned() or D is DType.bool:
            return self
        elif D.is_floating_point():
            ...
            if is_nvidia_gpu():
                ...
                @parameter
                if D.is_half_float():
                    alias prefix = "abs.bf16" if D is DType.bfloat16 else "abs.f16"
                    return _call_ptx_intrinsic[
            ...
            var mask = FPUtils[D].exponent_mantissa_mask()
    ...
    fn cast[target: DType](self) -> SIMD[target, size]:
        """Casts the elements of the SIMD vector to the target element D.
        ...
        if is_nvidia_gpu():
            ...
            @parameter
            if size > 1 and D is DType.float32 and target.is_half_float():
                # For size == 1, the LLVM backend generates the correct `cvt.rn.f16.f32`
                # instruction. This is why we do not handle it here.
                alias vector_asm_prefix = (
                    "cvt.rn.f16x2.f32" if target
                    is DType.float16 else "cvt.rn.bf16x2.f32"
                )
            ...
            elif D is DType.bfloat16 and target is DType.float64:
        ...
        @parameter
        if D in (DType.float8e4m3, DType.float8e5m2):
            constrained[
            ...
            if target is DType.float32:
                return _convert_float8_to_f32(rebind[SIMD[D, size]](self)).cast[
                    target
                ]()
            elif target is DType.float16:
                return _convert_float8_to_f16(rebind[SIMD[D, size]](self)).cast[
                    target
                ]()
        ...
        @parameter
        if has_neon() and (D is DType.bfloat16 or target is DType.bfloat16):
            # TODO(KERN-228): support BF16 on neon systems.
        ...
        @parameter
        if D is DType.bfloat16 and not _has_native_bf16_support():
    ...
    @always_inline
    fn to_bits[
        int_dtype: DType = _integral_type_of[D]()
        ...
        constrained[
            bitwidthof[int_dtype]() >= bitwidthof[D](),
            (
        ...
        return bitcast[_integral_type_of[D](), size](self).cast[int_dtype]()
    ...
    fn _floor_ceil_trunc_impl[intrinsic: StringLiteral](self) -> Self:
        ...
        @parameter
        if D is DType.bool or D.is_integral():
            return self
        elif has_neon() and D is DType.bfloat16:
            # TODO(KERN-228): support BF16 on neon systems.
    ...
    fn fma(self, multiplier: Self, accumulator: Self) -> Self:
        ...
        @parameter
        if (_is_sm_8x() or _is_sm_9x()) and D.is_half_float():
            alias prefix = "fma.rn.bf16" if D is DType.bfloat16 else "fma.rn.f16"
    ...
    fn _shuffle_variadic[
        *mask: Int, output_size: Int = size
    ](self, other: Self) -> SIMD[D, output_size]:
        ...
        return __mlir_op.`pop.simd.shuffle`[
            ...
            _type = __mlir_type[
                `!pop.simd<`, output_size.value, `, `, D.value, `>`
            ],
    ...
    fn _dynamic_shuffle[
        mask_size: Int, //
    ](self, mask: SIMD[DType.uint8, mask_size]) -> SIMD[D, mask_size]:
    ...
    fn slice[
        ...
        @parameter
        if offset % simdwidthof[D]():
            var tmp = SIMD[D, output_width]()
    ...
    @always_inline("nodebug")
    fn insert[*, offset: Int = 0](self, value: SIMD[D, _]) -> Self:
    ...
    @always_inline("nodebug")
    fn join(self, other: Self) -> SIMD[D, 2 * size] as result:
    ...
    @always_inline("nodebug")
    fn interleave(self, other: Self) -> SIMD[D, 2 * size]:
        ...
        @parameter
        if size == 1:
            return SIMD[D, 2 * size](self[0], other[0])
    ...
    alias _SIMDHalfType = SIMD[D, size // 2]
    ...
    @always_inline
    fn reduce[
        func: fn[D: DType, width: Int] (
            SIMD[D, width], SIMD[D, width]
        ) capturing -> SIMD[D, width],
        size_out: Int = 1,
    ](self) -> SIMD[D, size_out]:
        ...
        @parameter
        if size == size_out:
            return rebind[SIMD[D, size_out]](self)
        else:
    ...
    @always_inline("nodebug")
    fn reduce_max[size_out: Int = 1](self) -> SIMD[D, size_out]:
        ...
        if is_x86() or size_out > 1:
            ...
            fn max_reduce_body[
                D: DType, width: Int
            ](v1: SIMD[D, width], v2: SIMD[D, width]) -> SIMD[D, width]:
                return max(v1, v2)
        ...
        return rebind[SIMD[D, size_out]](
            llvm_intrinsic[
                "llvm.vector.reduce.smax", Scalar[D], has_side_effect=False
            ](self)
        )

    @always_inline("nodebug")
    fn reduce_min[size_out: Int = 1](self) -> SIMD[D, size_out]:
        ...
        return rebind[SIMD[D, size_out]](
            llvm_intrinsic[
                "llvm.vector.reduce.smin", Scalar[D], has_side_effect=False
            ](self)
        )

    @always_inline
    fn reduce_add[size_out: Int = 1](self) -> SIMD[D, size_out]:
    ...
    @always_inline
    fn reduce_mul[size_out: Int = 1](self) -> SIMD[D, size_out]:
    ...
    @always_inline
    fn reduce_and[size_out: Int = 1](self) -> SIMD[D, size_out]:
        ...
        return llvm_intrinsic[
            "llvm.vector.reduce.and", SIMD[D, size_out], has_side_effect=False
        ](self)

    @always_inline
    fn reduce_or[size_out: Int = 1](self) -> SIMD[D, size_out]:
        """Reduces the vector using the bitwise `|` operator.
        ...
        return llvm_intrinsic[
            "llvm.vector.reduce.or", SIMD[D, size_out], has_side_effect=False
        ](self)
    ...
    fn reduce_bit_count(self) -> Int:
        ...
        Constraints:
            Must be either an integral or a boolean D.
        ...
        if D is DType.bool:
            ...
            constrained[
                D.is_integral(), "Expected either integral or bool type"
            ]()
    ...
    fn select[
        ...
        """
        constrained[D is DType.bool, "the simd dtype must be bool"]()
...
@always_inline
fn _powi[D: DType](base: Scalar[D], exp: Int32) -> __type_of(base):
    if D.is_integral() and exp < 0:
        # Not defined for Integers, this should raise an
        # exception.
        debug_assert(False, "exponent < 0 is undefined for integers")
        return 0
    var a = base
    var b = abs(exp) if D.is_floating_point() else exp
    var res: Scalar[D] = 1
    while b > 0:
        if b & 1:
            res *= a
...
fn _convert_float8_to_f32_scaler[
    D: DType,
](x: Scalar[D]) -> Scalar[DType.float32]:
    var kF32_NaN: UInt32 = 0x7FFFFFFF
    var FP8_NUM_BITS = 8
    var IS_E4M3 = D is DType.float8e4m3
    var FP8_NUM_MANTISSA_BITS = FPUtils[D].mantissa_width()
    var FP8_NUM_EXPONENT_BITS = FPUtils[D].exponent_width()
...
fn _convert_float8_to_f32[
    D: DType,
    size: Int,
](val: SIMD[D, size]) -> SIMD[DType.float32, size]:
    @parameter
    if is_nvidia_gpu() and _is_sm_9x():
        return _convert_float8_to_f16(rebind[SIMD[D, size]](val)).cast[
            DType.float32
        ]()
    else:
        ...
        fn wrapper_fn[
            ...
            return rebind[Scalar[result_type]](
                _convert_float8_to_f32_scaler(rebind[Scalar[D]](val))
            )
...
fn _convert_float8_to_f16[
    D: DType,
    size: Int,
](val: SIMD[D, size],) -> SIMD[DType.float16, size]:
    @parameter
    if is_nvidia_gpu() and _is_sm_9x():
        alias asm_prefix = (
            "cvt.rn.f16x2.e4m3x2" if D
            is DType.float8e4m3 else "cvt.rn.f16x2.e5m2x2"
        )
        ...
        return _convert_float8_to_f32(rebind[SIMD[D, size]](val)).cast[
            DType.float16
        ]()
...
fn _modf(x: SIMD) -> Tuple[__type_of(x), __type_of(x)]:
    constrained[x.D.is_numeric(), "the type must be numeric"]()
...
fn _write_scalar[D: DType, W: Writer, //](inout writer: W, value: Scalar[D]):
    @parameter
    if D == DType.bool:
        if value:
            writer.write("True")
    ...
    # TODO: bring in modern int formatter and remove GPU specific code
        ...
        if is_gpu() or D.is_integral():
            ...
            alias size: Int = _calc_format_buffer_size[D]()
            var buf = InlineArray[UInt8, size](fill=0)
            var wrote = _snprintf[_get_dtype_printf_format[D]()](
