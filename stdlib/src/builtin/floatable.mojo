...
trait Floatable:
    ...
    Any type that conforms to `Floatable` works with the built-in `float` function.
...
trait FloatableRaising:
    """The `FloatableRaising` trait describes a type that can be converted to a Float,
    but the conversion might raise an error (eg: a string).
