...
struct _OldDynamicVector[type: AnyRegType](Sized):
    ...
    TODO: It should call its element destructors once we have traits.
    TODO: It should perform bound checks.
    ...
    fn __copyinit__(inout self, existing: Self):
        ...
        Args:
            existing: The `_OldDynamicVector` to copy.
        """
        self.data = existing.data
        self.size = existing.size
        self.capacity = existing.capacity
    ...
    fn __init__(inout self):
        ...
        self.data = Pointer[type].get_null()
        self.size = 0
        self.capacity = 0
    ...
    fn __init__(inout self, pointer: Pointer[type], size: Int):
    ...
    fn resize(inout self, size: Int):
        ...
        vector is appended with non-initialized elements up to the requested
        size.
    ...
    fn deepcopy(self) -> Self:
        ...
        Returns:
            The created copy of this vector.
        """
        var res = _OldDynamicVector[type](self.capacity)
        for i in range(len(self)):
            res.push_back(self[i])
        return res

    fn _realloc(inout self, new_capacity: Int):
        let new_data = Pointer[type].alloc(new_capacity)

        # TODO: Call actual memcpy
    ...
    fn pop_back(inout self) -> type:
        ...
        Returns:
            The popped value.
        """
        debug_assert(self.size > 0, "index must be positive")
        let ret_val = self.data.load(self.size - 1)
        self.size -= 1
        # If we are at 25% occupancy, half down the capacity
