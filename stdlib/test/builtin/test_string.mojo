...
fn test_comparison_operators() raises:
    ...
    assert_false("ab" >= s0)    # "ab" is not greater than or equal to String("abc")

    # Test comparisons involving default constructed empty string
    assert_true(default_string < s0)  # Assumes empty string is less than any non-empty
    assert_false(s0 < default_string)  # Non-empty string should not be less than an empty string
    assert_true(default_string <= s0)
    assert_false(s0 <= default_string)
    assert_true(default_string <= empty)  # Should be equal to explicitly empty string
