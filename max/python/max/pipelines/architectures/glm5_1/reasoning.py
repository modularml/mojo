...
"""GLM-4.5+ reasoning parser for ``<think>...</think>`` sections."""
...
class GlmReasoningParser(ReasoningParser):
    ...
    GLM's chat template appends ``<think>`` to every assistant turn via
    ...
    implicitly when a tool call begins (``<tool_call>``) — the tool-call marker
    ...
    def stream(
        ...
        if end_token_idx is None:
            ...
            # <tool_call> is not consumed — it belongs to the content region
            # where downstream tool parsing handles it.
            end_reasoning = end_token_idx
