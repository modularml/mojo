...
"""MiniMax-M2 tokenizer with reasoning-delimiter resolution."""
...
# Reasoning span delimiters. Both are special tokens in the MiniMax-M2
...
class MiniMaxM2Tokenizer(TextTokenizer):
    """:class:`TextTokenizer` with MiniMax-M2 reasoning-delimiter ids.

    Implements
    :class:`~max.pipelines.modeling.types.ReasoningPipelineTokenizer` so the
    overlap pipeline's thinking-mode temperature scaling can read the
    delimiter ids directly.
    """
    ...
    def reasoning_start_token_id(self) -> int:
        """Token id of ``<think>`` (opens a MiniMax-M2 reasoning span)."""
        return self._reasoning_start_token_id

    @property
    def reasoning_end_token_id(self) -> int:
        """Token id of ``</think>`` (closes a MiniMax-M2 reasoning span)."""
