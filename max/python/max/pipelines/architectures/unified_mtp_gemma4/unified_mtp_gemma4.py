...
class UnifiedMTPGemma4(Module):
    ...
    def __call__(
        ...
        with Graph.current.profile_scope(
            ...
            # -- 3. Target forward --
            # Vision embeddings are scattered into the target's token embeddings
            # at image_token_indices. Images are only present during prefill, where
        ...
        with Graph.current.profile_scope(
            ...
            # Compute q_max_seq_len for cross-attention kernel (uint32 scalar
            # on CPU)
            host_seq_lens = host_merged_offsets[1:] - host_merged_offsets[:-1]
            q_max_seq_len_prefill = (
                ops.max(host_seq_lens, axis=0)
                .cast(DType.uint32)
                .broadcast_to([1])
            )

            # -- 6. Draft step 0 (prefill): pass target hidden states to
            # draft --
            ...
            with Graph.current.profile_scope("draft_step_0"):
                ...
                self.draft.return_hidden_states = (
            ...
            input_lengths = ops.rebind(
                (input_row_offsets[1:] - input_row_offsets[:-1]).cast(
                    DType.int64
                ),
                ["batch_size"],
            )
            ...
            # The draft shares the target's KV; its logical cache length
            # equals the target sliding cache's. (Used only for the draft's
            # RoPE position, not for any cache write.)
            ...
            # merged prefill length), is_token_generation stays False, and
            # the depth512 pair-CTA prefill kernel is invoked for a 1-token
            # query -- a degenerate case that deadlocks under high KV-cache
            # pressure.
            ...
            for step in range(1, self.num_draft_steps):
                with Graph.current.profile_scope(f"draft_step_{step}"):
                    draft_hs = [
                        draft_hs[i].rebind(
                            [f"mtp_step{step}_batch", hidden_dim]
                        )
