...
"""Nemotron-H tokenizer with reasoning-delimiter resolution."""
...
# Reasoning span delimiters. Both are special tokens in the Nemotron-3
# tokenizer vocab; resolving them at init lets us implement the
# ``ReasoningPipelineTokenizer`` protocol that ``reasoning_parser="qwen3_5"``
# requires.
...
class NemotronHTokenizer(TextTokenizer):
    """A ``TextTokenizer`` that also exposes Nemotron's reasoning-delimiter ids.

    Implements the ``ReasoningPipelineTokenizer`` protocol so the overlap
    pipeline's thinking-mode tracking can read the ``<think>``/``</think>``
    ...
    def reasoning_start_token_id(self) -> int:
        """Token id of ``<think>`` (opens a Nemotron reasoning span)."""
        return self._reasoning_start_token_id

    @property
    def reasoning_end_token_id(self) -> int:
        """Token id of ``</think>`` (closes a Nemotron reasoning span)."""
