...
# ===----------------------------------------------------------------------=== #
"""Entry point for the ``grpc_worker`` binary.

Kept as a separate file from :py:mod:`.server` so that ``server.py`` lives
in exactly one Bazel rule (the :py:obj:`grpc_runtime` library) and can be
...
from max.experimental.cascade.grpc_runtime.server import cli
