...
# ===----------------------------------------------------------------------=== #
# Vendored from HuggingFace transformers v5.8.1:
...
class GlmMoeDsaAttention(nn.Module):
    ...
    **FP8 compatibility**: all weight accesses use standard nn.Linear forward calls (never
    ...
    def __init__(self, config: GlmMoeDsaConfig, layer_idx: int):
        ...
        self.next_skip_topk = (
            config.indexer_types[layer_idx + 1] == "shared"
            if layer_idx < len(config.indexer_types) - 1
            else False
        )
...
class GlmMoeDsaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, GlmMoeDsaTopkRouter):
            ...
            init.normal_(
                module.down_proj, mean=0.0, std=self.config.initializer_range
            )
