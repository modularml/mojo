...
# ===----------------------------------------------------------------------=== #
"""CPU producer: compile one sparse MLA prefill graph to a MEF with no GPU.
...
shared :func:`~test_common.mef_precompile.precompile_entrypoint`; this file only names
the graph builder to use (:func:`build_prefill_graph`, keyed by spec name).
"""

from __future__ import annotations

from _mla_sparse_graphs import SPECS_BY_NAME, build_prefill_graph
...
def _build(spec_name: str) -> Graph:
    ...
    return build_prefill_graph(spec)
