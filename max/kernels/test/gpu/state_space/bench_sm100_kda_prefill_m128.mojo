...
# ===----------------------------------------------------------------------=== #
"""Benchmark for the SM100 KDA prefill kernel, M128 variant.

Runs the four canonical shapes with the same methodology used to measure
the reference CUDA kernel on this box (device
events, 5 warmup iters, 30 timed iters, sync per iter, median, no L2 flush):

  shape                    original (eager)   original (kernel-only)
  H=96 fixed [8192]        0.615 ms           0.511 ms
  H=64 fixed [8192]        0.614 ms           0.504 ms
  H=96 uniform [1024]x8    0.541 ms           0.438 ms
  H=64 uniform [1024]x8    0.398 ms           0.297 ms

The two original columns differ because FlashInfer's eager Python wrapper
spends ~60-110 us of host time inside its event bracket (the stream idles
between the start event and the kernel launch); the kernel-only column is
the nsys/torch-profiler steady-state device time, i.e. what a CUDA-graph
deployment pays. This harness's bracket is a single enqueue (~5 us of
launch overhead), so its numbers sit ~5 us above this kernel's own
kernel-only time and should be compared against the original's
kernel-only column for cross-runtime claims; against the eager column
they show what an eager PyTorch caller sees. Within this harness the
bracket is the regression gate.
"""
...
def main() raises:
    ...
    print("shape                         median_ms   original_ms")
    var m1 = _bench_shape[96](ctx, 1, 8192)
    print("H=96 fixed [8192]           ", m1, "  0.615")
    var m2 = _bench_shape[64](ctx, 1, 8192)
    print("H=64 fixed [8192]           ", m2, "  0.614")
    var m3 = _bench_shape[96](ctx, 8, 1024)
    print("H=96 uniform [1024]x8       ", m3, "  0.541")
    var m4 = _bench_shape[64](ctx, 8, 1024)
    print("H=64 uniform [1024]x8       ", m4, "  0.398")
