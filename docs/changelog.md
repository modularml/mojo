...
- `Dict` now has a simplified conversion to `String` with `my_dict.__str__()`. Note that
  `Dict` does not conforms to the `Stringable` trait so `str(my_dict)` is not possible yet.
