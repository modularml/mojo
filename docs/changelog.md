...
- Added a new [`Box`](/mojo/stdlib/memory/box/Box) type as a safe, single-owner,
  non-nullable smart pointer with similar semantics to
  [`Rust's Box<>`](https://doc.rust-lang.org/std/boxed/struct.Box.html) and
  [`C++'s std::unique_ptr`](https://en.cppreference.com/w/cpp/memory/unique_ptr).
