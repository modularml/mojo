...
- Added a dedicated [reasoning](/serve/reasoning) serving guide covering
...
- Added documentation for audio generation:
  ...
  - Added a music generation example that renders songs past that per-render
    ...
    audible seams.
  - Documented audio generation in the REST API reference: a full
    `POST /v1/audio/speech` reference and the audio `provider_options` block
...
- Added Inkling (`InklingForConditionalGeneration`) support, a hybrid
...
- Changes to the dKV external KV-cache connector:
  ...
  - A request's `dkv_cache_hint` now reaches the dKV external KV-cache
    connector, which reads it to load a cached prefix from the instance that
    holds it rather than only from the co-located one. The serving layer
    forwards the field without interpreting it, so the hint schema is versioned
    in one place and a hint this build cannot use costs a cache miss rather than
    a failed request. Previously the field was parsed into a form nothing read,
    and every hinted load went to the co-located dKV.
...
- Added `MAX_SERVE_HTTP_KEEPALIVE_TIMEOUT_S` (default 120 seconds) so MAX
  Serve holds idle HTTP connections open longer than a pooling client's
...
- Added OpenTelemetry distributed tracing to MAX Serve: a
...
- Added APIs for unified host and device memory:
  - Added `Device.is_host_unified` (`max.driver`) and
    ...
    from one physical memory pool. Reports hardware topology, so it does not
    imply any given buffer is host-readable. Driver plugins answer it through
    the new optional `host_unified` device property.
...
- Improvements to MiniMax M3 kernels:
  - MiniMax-M3 sampled MTP now samples only the accepted initial draft row
    instead of every possible acceptance position.
  - The MiniMax M3 MSA sparse indexer on B200 now handles speculative
    query widths and is about 1.6x faster on decode.
...
- Improvements to Inkling kernels:
  - Sped up the sequence-parallel causal conv1d prefill kernel by 2.1× to
    3.1× at Inkling shapes on B200.
  - Fused the Inkling MoE router into a single kernel, cutting isolated
    routing from 33.4 µs to 12.9 µs.
...
- Fixed Inkling MTP initializing its config without the planned
  `max_seq_len`, which could ignore `--max-length`.
