...
* Derived origin. The `__origin_of()` magic function returns the origin associated
...
struct BoxedString:
    ...
    fn as_ptr(self) -> Pointer[String, __origin_of(self.o_ptr)]:
...
* An underscore character (`_`) to indicate that the origin is *unbound*. You
...
or immutable reference to a value. Like a `read` or `mut` argument, these
