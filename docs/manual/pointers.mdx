...
* A `Pointer` is *non-nullable*: it always points to something.
...
* A `Pointer` only refers to a single value. You can't do pointer arithmetic
  with a `Pointer`.
* A `Pointer` has an associated *origin*, which connects it back to an
...
references used with the `read` and `mut` argument conventions. Those
