...
manager and package manager (based on conda) that simplifies your development
workflow. With just a couple commands, the `magic` command-line tool creates a
new virtual environment that includes everything you need to get started with
Mojo.
...
1. Install Magic on macOS and Ubuntu with this command:
    ...
    Then run the appropriate `source` command for your terminal so that `magic`
...
2. Create a Mojo project called "hello-world":
    ...
    ```sh
    magic init hello-world --format mojo
    ```
...
```sh
magic init hello-world-nightly --format mojo \
